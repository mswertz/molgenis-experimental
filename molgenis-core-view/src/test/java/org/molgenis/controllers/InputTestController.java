...
public class InputTestController extends MolgenisController
{
	...
	public Div index()
	{
		return div(H1("Choose input to test:"), href("string", "string"), br(), href("integer", "integer"), br(),
				href("decimal", "decimal"), br(), href("bool", "bool"), br(), href("date", "date"), br(),
				href("datetime", "datetime"), br(), href("select", "select"), br(), href("xref", "xref"), br(),
				href("mref", "mref"), br(), href("email", "email"), br(), href("password", "password"), br(),
				href("text", "text"), br(), href("richtext", "richtext"), br(), href("file", "file"), br());
	...
	{
		return form(legend(input.getClass().getSimpleName() + " test form"), string("placeholder")
				.label("placeholder?").value(placeholder), help("Change and click OK to see placeholder"),
				bool("required").label("required?").value(required),
				help("Change and click OK to see required version"), input.required(required).placeholder(placeholder),
				post("ok", ""), href("back", "."));
	}
	...
	{
		return form(legend("BoolInput test form"), bool("value").value(value).label("bool").required(), post("ok", ""),
				href("back", "."));
	}
	...
	{
		return form(legend("Data input test form"), date("value").value(value).label("date").required(),
				post("ok", ""), href("back", "."));
	}
