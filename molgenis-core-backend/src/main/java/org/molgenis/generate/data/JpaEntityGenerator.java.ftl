...
<#-- INTERFACE OR CLASS?-->
<#if entity.abstract>
public interface ${JavaName(entity)} extends <#if entity.hasImplements()><#list entity.getImplements() as i> ${i.namespace}.${JavaName(i)}<#if i_has_next>,</#if></#list><#else>org.molgenis.data.IdentifiableRecord</#if>
<#else>
...
public class ${JavaName(entity)} <#if entity.hasExtends()>extends ${entity.extends.namespace}.${JavaName(entity.extends)}</#if><#if entity.hasImplements()>implements<#list entity.getImplements() as i> ${i.namespace}.${JavaName(i)}<#if i_has_next>,</#if></#list><#else> implements org.molgenis.data.IdentifiableRecord</#if>
</#if>
{
	// fieldname constants
...
<#else>
	...
	private ${JavaType(f)} ${name(f)} = ${default(f)};
	</#list>
