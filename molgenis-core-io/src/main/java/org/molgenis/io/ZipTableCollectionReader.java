...
public class ZipTableCollectionReader implements TableCollectionReader
{
	private final ZipFile zipFile;
	private final Map<String, ZipEntry> tableNameMap;

	public ZipTableCollectionReader(ZipFile zipFile) throws IOException
	{
	...
	{
		return Iterators.transform(tableNameMap.values().iterator(), new Function<ZipEntry, TableReader>()
		{
			@Override
			@Nullable
			public TableReader apply(@Nullable
			ZipEntry zipEntry)
			{
				try
				{
	...
	private TableReader toTupleReader(ZipEntry zipEntry) throws IOException
	{
