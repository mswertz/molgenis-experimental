...
public class ZipTableReader implements TableReader
{
	...
	{
		return Iterators.transform(tableNameMap.values().iterator(), new Function<ZipEntry, RecordReader>()
		{
			@Override
			@Nullable
			public RecordReader apply(@Nullable
			ZipEntry zipEntry)
			{
				try
				{
	...
	private RecordReader toTupleReader(ZipEntry zipEntry) throws IOException
	{
