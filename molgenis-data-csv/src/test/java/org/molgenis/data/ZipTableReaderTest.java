...
import org.molgenis.data.csv.CsvZipRepositoryCollection;
...
public class ZipTableReaderTest
{
	...
	{
		new CsvZipRepositoryCollection(null);
	}

	@Test
	public void getTableNames() throws IOException
	{
		CsvZipRepositoryCollection zipTableReader = new CsvZipRepositoryCollection(new ZipFile(ZIP_FILE));
		try
		{
			List<String> tableNames = Lists.newArrayList(zipTableReader.getRepositoryNames());
	...
	public void getTupleReader() throws IOException
	{
		...
		try
		{
			CsvReader tupleReader0 = zipTableReader.getRepository("0");
			try
			{
				Collection<String> colNames = Lists.newArrayList(tupleReader0.getMetaData().getFieldNames());
			...
			CsvReader tupleReader1 = zipTableReader.getRepository("1");
			try
			{
				Collection<String> colNames = Lists.newArrayList(tupleReader1.getMetaData().getFieldNames());
				assertEquals(2, colNames.size());
			...
			CsvReader tupleReader2 = zipTableReader.getRepository("2");
			try
			{
				Collection<String> colNames = Lists.newArrayList(tupleReader2.getMetaData().getFieldNames());
				assertEquals(2, colNames.size());
	...
	public void iterator() throws IOException
	{
		...
		try
		{
			Iterator<CsvReader> it = zipTableReader.iterator();
			assertTrue(it.hasNext());
			CsvReader tupleReader0 = it.next();
			assertNotNull(tupleReader0);
			tupleReader0.close();

			assertTrue(it.hasNext());
			CsvReader tupleReader1 = it.next();
			assertNotNull(tupleReader1);
			tupleReader1.close();

			assertTrue(it.hasNext());
			CsvReader tupleReader2 = it.next();
